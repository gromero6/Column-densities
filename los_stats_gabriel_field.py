...
def get_B_field_column_density(
    ...
    """
    This functions is supposed to calculate the column density over magnetic field lines for all x_init points.

    We do so first by finding the B field path for each x_init point, then, calculating the column densities foward
    and backward along the B field line, and finally, adding the two column densities together to get the total
    column density along the B field line.
    """
    BDtotal = np.zeros((max_cycles, 1))
    #outer loop over all x_init points
    for j in range(max_cycles): 
        ...
        current_fwd = x_init[j].copy()
        current_bck = x_init[j].copy()
        ...
        for i in range(N//2):
            ...
            column_fwd += local_densities_fwd * distance_traveled_fwd
        ...
        for i in range(N//2):
            ...
            local_densities_bck = local_densities_bck_arr[0]

            local_densities_bck *= gr_cm3_to_nuclei_cm3
            if local_densities_bck < densthresh:
                break

            dx_arr = np.array([dx])
            next_bck_arr, _, _, _, _, _ = Heun_step(
                np.array([current_bck]), dx_arr, -Bfield, Density, Density_grad, Pos, VoronoiPos, Volume
            )
            next_bck = next_bck_arr[0]
            
            distance_traveled_bck = np.linalg.norm(next_bck - current_bck) * pc_to_cm
            column_bck += local_densities_bck * distance_traveled_bck
            current_bck = next_bck    
        
        BDtotal[j] = column_fwd + column_bck
...
if __name__=='__main__':
    ...
    BDtotal = get_B_field_column_density(x_init,Pos,Bfield,Density,densthresh,N,max_cycles,Density_grad,VoronoiPos,Volume)
    ...
    np.savez(os.path.join(new_folder, f"DataBundle_MeanCDandBCD{seed}_{m}_{d}.npz"),
